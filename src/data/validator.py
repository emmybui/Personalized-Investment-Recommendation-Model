...
- Sort order:                 checked on (timestamp, transactionID).
...
from .config import PRIMARY_TEST_END
...
def validate_dataset(data):
    ...
    if tx["transactionID"].duplicated().any():
        errors.append("transactions: transactionID is not unique")
    ...
    # --- Transaction sort order: exact (timestamp, transactionID) check ---
    if not tx.empty and len(tx) > 1:
        expected = tx.sort_values(
            ["timestamp", "transactionID"], kind="stable"
        ).reset_index(drop=True)
        if not tx[["timestamp", "transactionID"]].reset_index(drop=True).equals(
            expected[["timestamp", "transactionID"]]
        ):
            errors.append(
                "transactions: not sorted by (timestamp, transactionID)")
    ...
    if not tx.empty and tx["timestamp"].max() < expected_end:
        warnings.append(
