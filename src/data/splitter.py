...
def _prepare_tx(tx):
    ...
    return tx.sort_values(EVENT_ORDER, kind="stable").reset_index(drop=True)
...
def build_rolling_splits(transactions, customers, assets, close_prices):
    ...
    for cutoff_text in ROLLING_CUTOFFS:
        # FAR-Trans timestamps are daily. Treat the named cutoff as end-of-day
        # so events on the cutoff date are history, never targets.
        cutoff = pd.Timestamp(cutoff_text) + pd.Timedelta(days=1) - pd.Timedelta(seconds=1)
...
def _split_stats(df, label, known_customers=None, known_assets=None):
    if df.empty:
        return {"split": label, "start": None, "end": None,
                "rows": 0, "customers": 0, "assets": 0,
                "unseen_customers": 0, "unseen_assets": 0}
    customer_ids = set(df["customerID"])
    asset_ids = set(df["ISIN"])
    return {
        "split": label,
        ...
        "unseen_customers": int(len(customer_ids - set(known_customers or []))),
        "unseen_assets": int(len(asset_ids - set(known_assets or []))),
    }
...
def save_primary_split(split: PrimarySplit, output_dir):
    ...
    # Summary
    train_customers = set(split.train["customerID"])
    train_assets = set(split.train["ISIN"])
    summary = pd.DataFrame([
        _split_stats(split.train, "train", train_customers, train_assets),
        _split_stats(split.validation, "validation", train_customers, train_assets),
        _split_stats(split.test, "test", train_customers, train_assets),
    ])
