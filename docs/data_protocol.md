...
- **Date range**: 2018-01-02 to 2022-11-29
...
| **Test** | 2022-07-01 — 2022-11-29 | Final evaluation (open LAST) |
...
```
ORDER BY timestamp ASC, transactionID ASC  (stable sort)
```
...
| Exception: 3 known FAR-Trans asset dual-classifications | Whitelisted, keep last |
...
All models (Popularity, BPR, LightGCN, TGN) use the same integer mapping:

```python
from src.data.loader import build_id_mapping

customer_to_idx, asset_to_idx = build_id_mapping(transactions)
```

The mapping is saved at `data/splits/id_mapping.json` during pipeline execution.

> **ID Mapping Bookkeeping Rule**:
> ID mapping is an index-only bookkeeping artifact. It does not use target values, interaction counts, features, or statistics from future periods. Original string IDs (`customerID`, `ISIN`) are ALWAYS preserved in raw/processed/split files; integer indices are used strictly for internal model array indexing across all baseline models and TGN.
